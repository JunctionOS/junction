...
func snapshot(elf string, metadata string) {
    ret := C.snapshot(C.CString(elf), C.CString(metadata))

    if ret == 0 {
        slog.Info("Snapshotted!", "ret", ret)
    } else {
        slog.Info("Restored!", "ret", ret)
    }
}
...
func main() {
    ...
    flag.BoolVar(&snapshotf, "snapshot", false, "whether to snapshot")
    flag.StringVar(&elf, "elf", "/tmp/resizer.elf", "elf to snapshot to")
    flag.StringVar(&metadata, "metadata", "/tmp/resizer.metadata", "metadata to snapshot to")
