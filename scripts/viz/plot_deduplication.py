...
def get_data_size(filename, readjif_path) -> int:
    ...
    result = subprocess.run(
        [readjif_path, filename, "jif.private_pages"], capture_output=True)
...
def dump_stats(values):
    values = sorted(
        zip(values[0], values[1], values[2], values[3]), key=lambda x: x[1])
    print('{:50}| Orig N pages | Deduped N pages |  Ratio'.format('Filename'))
    for (name, orig, new, relative) in values:
        print(
            '{:50}| {:12} | {:15} | {:6.2f}%'.format(
                name, orig, new, relative))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'orig',
        type=pathlib.Path,
        help='the directory of what will be considered the _original_ JIF files')
    parser.add_argument(
        'new',
        type=pathlib.Path,
        help='the directory of what will be considered the _new_ JIF files')
    parser.add_argument(
        'output',
        type=pathlib.Path,
    ...
    parser.add_argument(
        '--relative',
        action='store_true',
        help='whether to use relative or absolute comparison')
