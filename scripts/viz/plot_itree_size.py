...
def get_itree_sizes(filename, readjif_path):
    ...
    result = subprocess.run(
        [readjif_path, filename, "pheader.n_itree_nodes"], capture_output=True)
...
def get_stats(direc, readjif_path):
    aggregated = [get_itree_sizes(filename, readjif_path)
                  for filename in direc.glob('*.jif')]
    aggregated = sorted(aggregated, key=lambda x: x[0])
    aggregated = [
        ('Total', sorted(sum(map(lambda x: x[1], aggregated), [])))] + aggregated
    return aggregated


def print_stats(stats):
    for name, values in stats[1:]:
        print('{:50} | p25={:4} | median={:4} | p75={:4} | p99={:4} | max={:4}'.format(name,
                                                                                       values[len(values) // 4],
                                                                                       values[len(values) // 2],
                                                                                       values[(3 * len(values)) // 4],
                                                                                       values[(99 * len(values)) // 100],
                                                                                       values[-1]))
...
def violin_plot(stats):
    def color_idx(idx): return 'red' if idx == 0 else 'green'
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'output',
        type=pathlib.Path,
        help='the file to dump the plot in')
    parser.add_argument(
        '--total',
        action='store_true',
        help='whether to show only the total distribution')
